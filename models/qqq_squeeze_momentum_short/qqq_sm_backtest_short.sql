...
{% set total_row_number %}
    ...
    from {{ ref('qqq_squeeze_momentum_short') }}
{% endset %}
...
{{
  config(
    ...
    start_partition = 0,
    stop_condition = "select max(row_index) from analytics.qqq_squeeze_momentum_short",
    full_refresh = true,
  )
}}
...
with recursive thresholds AS (
    select
    	...
    	, 10 as momentum_threshold
    UNION ALL
    SELECT
    	CASE
      		WHEN momentum_threshold < 50 THEN squeeze_threshold
      	ELSE squeeze_threshold + 1
    	END,
    	CASE
      		WHEN momentum_threshold < 50 THEN momentum_threshold + 5
      	ELSE 10
    	END
  	FROM thresholds
...
status AS (
    ...
    FROM (
        ...
        FROM {{ ref('qqq_squeeze_momentum_short') }} AS sq -- Increase the limit to ensure enough data for the window functions
