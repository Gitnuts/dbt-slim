{{ config(
    ...
    WITH scaled_cumreturn AS (
        ...
        from (
            select
                *,
                row_index as rn,
                case when STDDEV(cumulative_return) OVER (PARTITION BY row_index) > 0
                    then (cumulative_return - AVG(cumulative_return) OVER (PARTITION BY row_index)) / STDDEV(cumulative_return) OVER (PARTITION BY row_index)
                else null end as scaled_return
            FROM (select * from {{ ref('iwm_sm_backtest') }} order by squeeze_threshold, momentum_threshold, finish_timestamp)
            WHERE squeeze_threshold < 17
            order by row_index, squeeze_threshold, momentum_threshold
        ) as scaled_cumreturn_no_index
        where scaled_return is not null
    ),
    perc as (select
        rn,
    ...
    select
        ...
        ceil(round(avg(momentum_threshold)*100, 0) / 3) * 3 / 100 as momentum_threshold_centroid,
        outcome,
