...
{{
  config(
    ...
    start_partition = 0,
    stop_condition = "select max(row_index) from analytics.iwm_squeeze_momentum",
    full_refresh = true,
  )
}}
...
with recursive thresholds AS (
    ...
    SELECT
    	CASE
      		WHEN momentum_threshold < 33 THEN squeeze_threshold
      	ELSE squeeze_threshold + 1
    	END,
    	CASE
      		WHEN momentum_threshold < 33 THEN momentum_threshold + 3
      	ELSE 6
    	END
  	FROM thresholds
...
status AS (
    ...
    FROM (
        ...
        FROM {{ ref('iwm_squeeze_momentum') }} AS sq -- Increase the limit to ensure enough data for the window functions
