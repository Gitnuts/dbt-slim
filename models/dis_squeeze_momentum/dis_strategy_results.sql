{{ config(
    ...
    WITH scaled_cumreturn AS (
        ...
        from (
            ...
            FROM (select * from {{ ref('dis_sm_backtest') }} order by squeeze_threshold, momentum_threshold, finish_timestamp)
    ...
    select
        ...
        round(avg(momentum_threshold)/10, 2) * 10 as momentum_threshold_centroid,
        outcome,
