...
{{
  config(
    ...
    start_partition = 0,
    stop_condition = "select max(row_index) from analytics.dis_squeeze_momentum",
    full_refresh = true,
  )
}}
...
with recursive thresholds AS (
    select
    	...
    	, 15 as momentum_threshold
    UNION ALL
    SELECT
    	CASE
      		WHEN momentum_threshold < 55 THEN squeeze_threshold
      	ELSE squeeze_threshold + 1
    	END,
    	CASE
      		WHEN momentum_threshold < 55 THEN momentum_threshold + 5
      	ELSE 15
    	END
  	FROM thresholds
...
status AS (
    ...
    FROM (
        ...
        FROM {{ ref('dis_squeeze_momentum') }} AS sq -- Increase the limit to ensure enough data for the window functions
